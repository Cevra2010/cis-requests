    ...
    <div class="mt-6">
        ...
        <div class="space-y-4">
            ...
            @php
                ...
                $progress   = $receipt ? $receipt->progress() : [0, $source->expectedCount];
                $isExpanded = $expandedSourceId === $source->cis_row_id;
                $openItems   = $receipt ? $receipt->items->filter(fn($i) => $i->isOpen())->sortBy(fn($i) => $i->position?->sort_order ?? 0) : collect();
                $closedItems = $receipt ? $receipt->items->filter(fn($i) => $i->isClosed())->sortBy(fn($i) => $i->position?->sort_order ?? 0) : collect();
            @endphp
            <div class="cis-card" wire:key="source-{{ $source->cis_row_id }}">
                <div class="flex items-center justify-between gap-4 flex-wrap">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-sky-50 flex items-center justify-center shrink-0">
                            <i class="fa fa-house text-sky-500 text-sm"></i>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-800">{{ $source->name }}</p>
                            <p class="text-xs text-gray-400">{{ $source->expectedCount }} Position(en) · nicht ausschreibungsrelevant</p>
                        </div>
                    </div>

                    <div class="flex items-center gap-2 flex-wrap">
                        @if(! $receipt)
                            <button type="button" wire:click="startInternalReceipt('{{ $source->cis_row_id }}')" class="btn btn-primary btn-sm">
                                <i class="fa fa-clipboard-check mr-1.5"></i> Wareneingang starten
                            </button>
                        @else
                            @if($receipt->isComplete())
                            ...
                            <button type="button" wire:click="toggleExpandedSource('{{ $source->cis_row_id }}')" class="btn btn-ghost btn-sm">
                                <i class="fa fa-{{ $isExpanded ? 'chevron-up' : 'chevron-down' }} mr-1.5"></i>
                                {{ $isExpanded ? 'Einklappen' : 'Details' }}
                            </button>
                        @endif
                        ...
                        <div class="overflow-x-auto rounded-lg border border-gray-100">
                            <table class="min-w-full text-sm">
                                <thead>
                                    <tr class="bg-gray-50 border-b border-gray-100">
                                        <th class="text-left text-[10px] font-bold uppercase tracking-wider text-gray-400 px-3 py-2">Position</th>
                                        <th class="text-center text-[10px] font-bold uppercase tracking-wider text-gray-400 px-2 py-2 w-20">Erwartet</th>
                                        <th class="text-center text-[10px] font-bold uppercase tracking-wider text-gray-400 px-2 py-2 w-20">Erhalten</th>
                                        <th class="text-left text-[10px] font-bold uppercase tracking-wider text-gray-400 px-3 py-2">Zuletzt von</th>
                                    </tr>
                                </thead>
