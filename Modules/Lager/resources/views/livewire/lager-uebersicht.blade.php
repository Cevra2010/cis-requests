<div>
    ...
    <div class="cis-card mb-4">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Produkt suchen…" class="cis-input text-sm">
            <select wire:model.live="lagerortFilter" class="cis-input text-sm">
                <option value="">Alle Lagerorte</option>
                @foreach($lagerorte as $l)
                    <option value="{{ $l->cis_row_id }}">{{ str_repeat('— ', $l->depth) }}{{ $l->name }}</option>
                @endforeach
            </select>
            <select wire:model.live="projectFilter" class="cis-input text-sm">
                <option value="">Alle Projekte</option>
                <option value="none">Nicht zugeordnet</option>
                @foreach($projects as $p)
                    <option value="{{ $p->cis_row_id }}">{{ $p->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
